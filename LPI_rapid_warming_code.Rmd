...
colnames(spid) <-
1950:2005              #renaming the date column names as R doesn't like numbered column names
name <- ids$Binomial
id <- ids$ID
points <- ids$points_per_pop1950_2005
name_id <-
paste(name, id, sep = "_") #creating id for naming files of plots
Date <- as.numeric(colnames(spid))
# spidt <- destring(t(spid))
spidt <- as.numeric(as.character(spid))
time <-
length(min(which(!is.na(spidt))):max(which(!is.na(spidt))))
missing <- time - points

Year <- Date[min(which(!is.na(spidt))):max(which(!is.na(spidt)))]
Population <-
spidt[min(which(!is.na(spidt))):max(which(!is.na(spidt)))]
Population[Population == 0] <-
mean(Population, na.rm = TRUE) * 0.01 #if a population is zero one year thhis is replaced with 1% of the average population estimate - because you can log zeros
...
first <- gsub("X","",names(CR)[1])

last <-  gsub("X","",names(CR)[length(names(CR))])
  
dates <- seq(ymd(first), ymd(last), by = 'months')

datesr <- rep(dates, each=length(xy_df))   #each number should be number of pops
...
srm3 <- srm2[,c("lon","lat","datesr","value")]
...
write.csv(LPIclim, "Global_Mean_Monthly_Temp.csv", row.names = FALSE)
